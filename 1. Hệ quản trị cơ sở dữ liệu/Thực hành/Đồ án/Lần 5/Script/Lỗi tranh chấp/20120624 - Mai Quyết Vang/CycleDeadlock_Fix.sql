...
begin transaction
	...
	begin try
		...
		If exixts (SELECT * FROM CHITIETDONDATHANG WHERE MaDH = @MaDH and MaTP = @MaTP and MaDT = @MaDT 
...
begin transaction
	...
	-- IF EXISTS (SELECT * FROM CHITIETDONDATHANG WHERE MaDH = @MaDH and MaTP=@MaTP and MaDT=@MaDT)
	BEGIN
		UPDATE CHITIETDONDATHANG   
		SET SoLuong = @SoLuong
		WHERE MaDH = @MaDH and MaTP = @MaTP and MaDT = @MaDT
	...
	--ELSE 
	--BEGIN
	--	INSERT INTO CHITIETDONDATHANG   with (HOLDLOCK)
